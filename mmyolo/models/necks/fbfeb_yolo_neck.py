...
from mmyolo.models.layers.fbfeblock import FFEBlock, BFEBlock
from mmyolo.registry import MODELS



@MODELS.register_module()
class FBFEBYOLONeck(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if self.ffeb_cfg['ffeb_use']:
            self.ffeb_block = FFEBlock(ffeb_use=self.ffeb_cfg['ffeb_use'], 
                                        channels_list=self.ffeb_cfg['channels_list'],
                                        num_repeats=self.ffeb_cfg['num_repeats'],
                                        extra_cfg=self.ffeb_cfg['extra_cfg'])
 
        self.bfeb_cfg = bfeb_cfg
        if self.bfeb_cfg['bfeb_use']:
            self.bfeb_block = BFEBlock(bfeb_use=self.bfeb_cfg['bfeb_use'], 
                                        channels_list=self.bfeb_cfg['channels_list'],
                                        num_repeats=self.bfeb_cfg['num_repeats'],
                                        extra_cfg=self.bfeb_cfg['extra_cfg'])
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        # bfeb_block
        if self.bfeb_cfg['bfeb_use']:
            results = self.bfeb_block(results)
